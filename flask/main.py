...
def recommend():
    ...
    idd = r.select("movies_id").rdd.flatMap(lambda x: x).collect()
    # input_id = r.select("input_movies_id").rdd.flatMap(lambda x: x).collect()
    ...
    if type(r) == type('string'):
        ...
        # print(input_id)
        # print(idd)
        # raw_df = df_merge.filter(col("id") == input_id[0]).select(
        #     col('id'), col('imdb_id'), col('vote_count'), col(
        #         'vote_average'), col('release_date'),
        #     col('status'), col('time'), col('genres'), col('overview'), col('popularity'))
        # m_id = input_id[0]
        ...
        for id in idd:
            ...
            response = requests.get(
                'https://api.themoviedb.org/3/movie/{}?api_key={}'.format(id, tmdb.api_key))
            data_json = response.json()
            pic = data_json['poster_path']
            posters.append('https://image.tmdb.org/t/p/original' + pic)
            ids.append(id)
            print(ids)
