...
@asset(
    description="Data from silver layer to gold layer",
    ins={
        "silver_merge_df": AssetIn(
            key_prefix=["silver", "final"]
        ),
    ...
    io_manager_key="spark_io_manager",
    key_prefix=[LAYER, "merge_movies"],
    metadata={"mode": "overwrite"},
    compute_kind=COMPUTE_KIND,
    group_name=LAYER
)
def gold_merge_movies(context, silver_merge_df: DataFrame) -> Output[DataFrame]:

    spark_df = silver_merge_df.select('id', 'imdb_id', 'budget', 'genres_convert', 'original_language', 
